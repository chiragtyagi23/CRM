...
const compactInputClass =
  'w-full h-8 min-w-0 px-2 rounded-md border border-[#E8DCCB] bg-white text-xs text-[#2E2E2E] placeholder:text-[#8B7355] focus:outline-none focus:ring-2 focus:ring-violet-400/20 focus:border-violet-300/60'
...
export function CampaignUploadTile({
  ...
  if (compact) {
    return (
      <div className="rounded-lg border border-[#E8DCCB] bg-white p-2 shadow-sm">
        <div className="mb-1.5 flex items-center justify-between gap-2">
          <span className="truncate text-xs font-semibold text-[#2E2E2E]">{label}</span>
          <div className="flex shrink-0 flex-wrap items-center justify-end gap-1">
            {value?.file ? (
              <button
                type="button"
                onClick={clearLocalSelection}
                className="h-7 whitespace-nowrap rounded-md border border-[#E8DCCB] bg-white px-2 text-[11px] font-semibold text-[#2E2E2E] hover:bg-[#F5EFE7]"
              >
                Clear
              </button>
            ) : null}
        ...
        <div className="flex gap-2">
          <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-md border border-[#E8DCCB] bg-gray-50">
            {value?.src ? (
              <img src={value.src} alt={value.alt || label} className="h-full w-full object-cover" />
            ) : value?.file ? (
              <div className="flex h-full items-center justify-center p-1 text-center text-[10px] leading-tight text-[#8B7355]">File</div>
            ) : (
              <div className="flex h-full items-center justify-center text-[10px] text-[#8B7355]">—</div>
            )}
          </div>
          <div className="min-w-0 flex-1 space-y-1.5">
              ...
              <>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <label
                    ...
                    className={
                      uploading
                        ? 'inline-flex h-7 cursor-wait items-center rounded-md border border-[#E8DCCB] bg-gray-100 px-2 text-[11px] font-semibold text-[#8B7355]'
                        : 'inline-flex h-7 cursor-pointer items-center rounded-md border border-[#E8DCCB] bg-white px-2 text-[11px] font-semibold text-[#2E2E2E] hover:bg-[#F5EFE7]'
                    }
                  >
                    {uploadMode === 'defer' ? 'Upload' : uploading ? '…' : 'Upload'}
                  </label>
                  {value?.file ? (
                    <span className="max-w-[140px] truncate text-[10px] text-[#8B7355]" title={value.file.name}>
                      {value.file.name}
                    </span>
                  ) : null}
                </div>
            ...
            {uploadError ? <div className="text-[11px] text-[#D96B6B]">{uploadError}</div> : null}
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="rounded-xl border border-dashed border-[#E8DCCB] bg-gray-50 p-4">
        ...
        <div>
          ...
          {value?.file ? (
            <div className="mt-2 text-[11px] font-semibold text-[#2E2E2E]">
              ...
              {isLikelyTiff(value.file) ? <span className="ml-2 font-normal text-[#8B7355]">(TIFF preview generated locally)</span> : null}
            </div>
          ) : null}
        </div>
        <div className="flex flex-wrap items-center justify-end gap-2">
          {value?.file ? (
            <button type="button" onClick={clearLocalSelection} className="h-8 whitespace-nowrap px-3 rounded-lg border border-[#E8DCCB] bg-white text-[#2E2E2E] text-xs font-semibold hover:bg-[#F5EFE7]">
              Unselect
            </button>
          ) : null}
          {onRemove ? (
            <button type="button" onClick={onRemove} className="h-8 whitespace-nowrap px-3 rounded-lg border border-[#E8DCCB] bg-white text-[#2E2E2E] text-xs font-semibold hover:bg-[#F5EFE7]">
              Remove
            </button>
          ) : null}
        </div>
      ...
      {onChange ? (
        <div className="mt-3">
          <input id={inputId} type="file" accept="image/*" className="hidden" disabled={uploading} onChange={handleFileChange} multiple={Boolean(allowMultiple)} />
          <label htmlFor={inputId} className={uploading ? 'inline-flex items-center justify-center h-9 px-3 rounded-lg border border-[#E8DCCB] bg-gray-100 text-[#8B7355] text-sm font-semibold cursor-wait' : 'inline-flex items-center justify-center h-9 px-3 rounded-lg border border-[#E8DCCB] bg-white text-sm font-semibold text-[#2E2E2E] hover:bg-[#F5EFE7] cursor-pointer'}>
